...
export default function UserManagementPanel({ primaryColor, onBack }: Props) {
  ...
  const load = async () => {
    try {
      ...
      const [usersRes, boothsRes] = await Promise.all([api.get('/users/management-users', { params: { role: tab, page, pageSize, search: search || undefined, status: status || undefined } }), api.get('/booths')]);
  ...
  useEffect(() => { void load(); setSelected(null); }, [tab, page, search, status]);
  ...
  return <View style={styles.container}><ScrollView contentContainerStyle={styles.content}>
    ...
    {tab === 'recepcao_level_3' ? <TouchableOpacity style={[styles.primary, { backgroundColor: primaryColor, width: '100%', maxWidth: 620, marginBottom: 14 }]} onPress={() => setShowCreate((value) => !value)}><Text style={styles.white}>{showCreate ? 'Fechar cadastro' : 'Cadastrar Recepção'}</Text></TouchableOpacity> : null}
    <View style={styles.scalableControls}><TextInput value={search} onChangeText={(value) => { setPage(1); setSearch(value); }} placeholder="Buscar por nome, nome de guerra ou e-mail" style={styles.input} /><View style={styles.filterRow}>{['', 'active', 'inactive'].map((value) => <TouchableOpacity key={value || 'all'} style={[styles.filterButton, status === value && { backgroundColor: primaryColor }]} onPress={() => { setPage(1); setStatus(value); }}><Text style={status === value ? styles.white : styles.filterText}>{value === '' ? 'Todos' : value === 'active' ? 'Ativos' : 'Inativos'}</Text></TouchableOpacity>)}</View></View>
    {showCreate && tab === 'recepcao_level_3' ? <View style={styles.editor}><Text style={styles.section}>Nova Recepção</Text><Text style={styles.label}>Nome completo</Text><TextInput style={styles.input} value={newName} onChangeText={setNewName} placeholder="Nome completo" /><Text style={styles.label}>Nome de Guerra</Text><TextInput style={styles.input} value={newNomeGuerra} onChangeText={(value) => setNewNomeGuerra(value.toLocaleUpperCase('pt-BR'))} placeholder="NOME DE GUERRA" autoCapitalize="characters" /><Text style={styles.label}>E-mail de acesso</Text><TextInput style={styles.input} value={newEmail} onChangeText={setNewEmail} placeholder="email@empresa.com" keyboardType="email-address" autoCapitalize="none" /><Text style={styles.label}>Senha temporária</Text><Text style={styles.help}>Mínimo de 8 caracteres. A troca será obrigatória no primeiro acesso.</Text><TextInput style={styles.input} value={newPassword} onChangeText={setNewPassword} secureTextEntry autoCapitalize="none" /><Text style={styles.section}>Plantões autorizados no cadastro</Text><Text style={styles.help}>Selecione agora um ou mais plantões. Essa atribuição também poderá ser alterada depois no card da Recepção.</Text>{booths.length === 0 ? <Text style={styles.help}>Nenhum plantão disponível para atribuição.</Text> : booths.map((booth) => <TouchableOpacity key={booth.id} style={[styles.booth, newAssigned.includes(booth.id) && { backgroundColor: '#dcfce7', borderColor: '#15803d' }]} onPress={() => setNewAssigned((current) => current.includes(booth.id) ? current.filter((id) => id !== booth.id) : [...current, booth.id])} disabled={saving}><Text style={styles.line}>{newAssigned.includes(booth.id) ? '✓ ' : ''}{booth.name}</Text></TouchableOpacity>)}<TouchableOpacity style={[styles.primary, { backgroundColor: primaryColor }]} onPress={() => void createReceptionist()} disabled={saving}><Text style={styles.white}>Criar Recepção</Text></TouchableOpacity></View> : null}
